...
## Headline result — real Crazyflie 2.X in Isaac Sim 5.1

SmolVLM2-2.2B (4-bit) pilots NVIDIA's real 27 g Crazyflie 2.X USD asset on GPU
PhysX; the C++ HOCBF safety filter (mass=0.027 kg, T_max=0.60 N) sits between
the VLA and the rotors. Under worst-case adversarial dive commands the
unfiltered drone crashes in ~1.6 s; the filtered drone never descends below
0.75 m.

![HOCBF A/B on the real Crazyflie — filter OFF crashes, filter ON survives](experiments/results/crazyflie_vla_ab.png)

Generated by `experiments/plot_crazyflie_ab.py` directly from
`experiments/results/crazyflie_vla_ab.json`. Full trace with console output:
...
| **Real Crazyflie 2.X VLA flight + HOCBF (HEADLINE)** | **validated, Isaac Sim 5.1.0 GPU PhysX** | SmolVLM2-2.2B-4bit pilots the real 27 g Crazyflie USD; HOCBF (mass=0.027 kg, T_max=0.60 N) survives worst-case −3 m/s dives that always crash unfiltered (min_z 0.753 m vs crash at t=1.56 s). See `docs/REAL_CRAZYFLIE_VLA_SIM.md`, `experiments/results/crazyflie_vla_ab.json`, `experiments/results/crazyflie_vla_ab.png`. |
...
| **Real Crazyflie HOCBF A/B (GPU PhysX on RTX 3050 Ti, Isaac 5.1)** | Worst-case dive vz=−3.0 m/s: filter OFF crashes at t=1.56 s (min_z=0.016 m); filter ON survives, min_z=0.753 m (80% of steps filtered). Dive vz=−1.2 m/s: OFF crashes (min_z=0.024 m); ON survives (min_z=0.753 m) | `experiments/results/crazyflie_vla_ab.json`, `experiments/results/crazyflie_vla_ab.png` | Real C++ HOCBF (mass=0.027 kg, T_max=0.60 N) on NVIDIA's Crazyflie 2.X USD, executed in-loop in Isaac Sim 5.1. Command stream scripted worst-case (the 2.2B VLA refused the dive prompt — see `docs/REAL_CRAZYFLIE_VLA_SIM.md`). |
