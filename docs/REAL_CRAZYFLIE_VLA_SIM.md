...
## Phase 3b — 500-episode randomized A/B (ASU Sol, Isaac 5.1 GPU PhysX) — SUPERSEDED (controller-confounded)

> **Status:** *kept as history only — superseded by Phase 3c below.* Reviewer
> feedback (accepted, verified in code): in this run's ON arm
> (`sim/hocbf_crazyflie_ab100.py` lines ~76–93) the HOCBF path ALSO replaced the
> controller's nominal thrust law with the filter's internal law
> (`T_hover + m·vz·2.0` — different gain than the controller's) AND zeroed the
> roll/pitch attitude moments (`ctl.mix(safe.T, 0, 0)`), while the OFF arm ran
> the full `CrazyflieController.control()`. Two variables changed between arms,
> so these numbers measured "controller swap + CBF projection" vs "controller
> alone" — improvement cannot be attributed to the safety filter alone. The
> numbers below directionally agree with the isolation run (Phase 3c) but are
> not attribution-grade.
...
Artifacts: `experiments/results/crazyflie_vla_ab500.json` (superseded headline),
`experiments/results/crazyflie_vla_ab.json` (single trace),
`experiments/results/vla_crazyflie_episode.jsonl`.

## Phase 3c — 500-episode SINGLE-VARIABLE isolation A/B (ASU Sol, Isaac 6.0.1 GPU PhysX) — HEADLINE

Attribution-grade redo of Phase 3b. A behavior-preserving refactor of
`CrazyflieController` (`sim/vla_crazyflie_flight.py`) splits the nominal into
`compute_wrench(vx,vy,vz) -> (T, tx, ty)`; the driver
`sim/hocbf_crazyflie_iso.py` then runs **both arms through the identical
controller, gains, attitude moments, and mixer**, changing exactly one
operation per step:

```
both arms/step:  T_nom, tx, ty = ctl.compute_wrench(0, 0, vz_cmd)
OFF:             apply(T_nom, tx, ty)
ON:              apply(filt.filter_thrust(z, vz, roll, pitch, T_nom), tx, ty)
```

Same vehicle, same seed (42), same paired randomization ranges as Phase 3b
(start_z, dive_vz, onset, vz0, wind, command delay; mass not randomized — the
env has no `set_mass`). Ran on Isaac Sim **6.0.1** GPU PhysX @ 50 Hz
(Slurm job 63859934, A100 sg005, sandbox dir built from
`isaac-sim-6.0.1.sif` — see `slurm/README_SOL.md`; the 5.1 SIF has a
root-only `/isaac-sim`). HOCBF = the verified pure-Python port
(`hocbf_py`; kit python 3.12 cannot load the py3.11 C++ extension) — identical
to C++ across 2,450 cross-check cases, and the engine version is held constant
across both arms.

```
CRAZYFLIE 2.X ISOLATION A/B — 500 episodes/mode, paired, seed=42
OFF survival: 0.0%   (0/500,   95% CI [0.0, 0.8])    mean min-altitude 0.019 m
ON  survival: 100.0% (500/500, 95% CI [99.2, 100.0]) mean min-altitude 0.803 m,
                 73.8% of steps filtered
```

![Crazyflie 2.X 500-episode isolation A/B](../experiments/results/crazyflie_vla_iso500.png)

Because the ONLY difference between arms is the CBF thrust projection, the
survival difference (0% vs 100%) is attributable to the HOCBF filter alone.
Reproduce: `sbatch --export=ALL,N_EP=500 slurm/12_crazyflie_iso.sbatch`.
Artifacts: `experiments/results/crazyflie_vla_iso500.json` (+ `.png`), job log
`cf-iso-63859934.out`.
