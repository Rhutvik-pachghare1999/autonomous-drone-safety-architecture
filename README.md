...
**Headline result — 500-episode randomized A/B (ASU Sol, Isaac Sim 5.1 GPU PhysX):**
across 500 paired, domain-randomized episodes (start altitude, dive velocity,
onset, wind, command delay) under worst-case adversarial dive commands, the
**filter OFF drone crashes in 500/500 episodes (0% survival, mean min-altitude
0.019 m); filter ON survives 500/500 (100% survival, mean min-altitude 0.782 m,
88.7% of steps filtered).**

![Crazyflie 2.X HOCBF A/B — 500 episodes: filter OFF 0% vs ON 100% survival](experiments/results/crazyflie_vla_ab500.png)

Generated by `experiments/plot_crazyflie_ab_survival.py` directly from
`experiments/results/crazyflie_vla_ab500.json` (no fabricated data). A single
illustrative dive trace (filter OFF crashes at ~1.6 s; ON holds > 0.75 m) is in
`experiments/results/crazyflie_vla_ab.json`. Full run details, honest scope, and
console output: [`docs/REAL_CRAZYFLIE_VLA_SIM.md`](docs/REAL_CRAZYFLIE_VLA_SIM.md).
...
> prompts). The survival A/B uses a pure-Python HOCBF port verified identical to
